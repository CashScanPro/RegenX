    ...
    <div className="flex min-h-screen" style={{ backgroundColor: '#0a0a0a', color: '#fff' }}>
      ...
      <main style={{ marginLeft: '250px', flex: 1, padding: '3rem 2.5rem' }}>
        <div className="rx-fade-up" style={{ marginBottom: '2.5rem' }}>
          <div className="rx-eyebrow" style={{ marginBottom: '0.6rem' }}>★ Espace Membre</div>
          <h1 style={{ fontSize: '2.25rem', fontWeight: 900, letterSpacing: '-0.03em' }}>Mon compte</h1>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '1.5rem', maxWidth: '960px' }}>
          {/* Profile card */}
          <div className="rx-card rx-fade-up">
            <div className="rx-eyebrow" style={{ color: 'rgba(255,255,255,0.3)', letterSpacing: '0.2em', marginBottom: '1.5rem' }}>Profil</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
              <div style={{ width: '60px', height: '60px', background: 'linear-gradient(135deg, ' + GOLD + ', ' + GOLD_LIGHT + ')', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.4rem', fontWeight: 900, color: '#0a0a0a', flexShrink: 0, boxShadow: '0 4px 16px rgba(200,146,42,0.3)' }}>
                {avatarLetter}
              </div>
              <div>
                <div style={{ fontSize: '1.05rem', fontWeight: 800 }}>{displayName}</div>
                <div style={{ fontSize: '0.78rem', color: 'rgba(255,255,255,0.4)' }}>{email}</div>
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {[['Membre depuis', createdAt], ['Statut', 'Actif']].map(([l, v]) => (
                <div key={l} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '0.85rem', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                  ...
                  <span style={{ fontSize: '0.85rem', fontWeight: 700, color: l === 'Statut' ? '#4ade80' : '#fff' }}>{v}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Subscription card */}
          <div className="rx-card-gold rx-fade-up">
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.5rem' }}>
              <Crown style={{ width: '17px', height: '17px', color: GOLD }} />
              <div className="rx-eyebrow" style={{ letterSpacing: '0.2em' }}>Abonnement</div>
            </div>
            <div style={{ marginBottom: '1.5rem' }}>
              <div style={{ fontSize: '1.7rem', fontWeight: 900, color: GOLD, letterSpacing: '-0.02em' }}>Premium</div>
              <div style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.45)', marginTop: '4px' }}>Accès illimité — Coach IA 24h/24</div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.7rem', marginBottom: '1.75rem' }}>
              {['Coach IA illimitée', 'Programmes personnalisés', 'Plans nutritionnels', 'Suivi avancé', 'Support prioritaire'].map(f => (
                <div key={f} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', fontSize: '0.82rem', color: 'rgba(255,255,255,0.65)' }}>
                  <Check style={{ width: '15px', height: '15px', color: GOLD, flexShrink: 0 }} />{f}
                </div>
              ))}
            </div>
            <Link href="/pricing" className="rx-btn-gold" style={{ width: '100%' }}>
          ...
          <div className="rx-card rx-fade-up" style={{ gridColumn: '1 / -1' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.25rem' }}>
              <Shield style={{ width: '15px', height: '15px', color: 'rgba(255,255,255,0.35)' }} />
              <div className="rx-eyebrow" style={{ color: 'rgba(255,255,255,0.3)', letterSpacing: '0.2em' }}>Données & confidentialité</div>
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem' }}>
              {[['Mentions légales', '/mentions-legales'], ['Politique de confidentialité', '/privacy'], ['CGU', '/terms']].map(([l, h]) => (
                <Link key={l} href={h} className="rx-btn-ghost">
