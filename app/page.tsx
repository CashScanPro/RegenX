...
const plans = [
  { key: 'starter' as PlanKey, name: 'Starter', price: '29', period: 'mois', desc: 'Pour démarrer votre transformation', features: ['IA Coach 2h/jour', 'Programmes sport de base', 'Plans nutritionnels simples', 'Suivi progression basique', 'App mobile incluse'], highlight: false },
  { key: 'pro' as PlanKey, name: 'Pro', price: '99', period: 'mois', desc: 'Pour les passionés qui veulent des résultats', features: ['IA Coach illimitée 24h/24', 'Programmes sport personnalisés', 'Plans nutritionnels adaptés', 'Suivi progression avancé', 'Support prioritaire'], highlight: true, badge: 'Populaire' },
  { key: 'equipe' as PlanKey, name: 'Équipe', price: '149', period: 'mois', desc: 'Pour les coachs et équipes sportives', features: ['Tout le forfait Pro', 'Tableau de bord coach', 'Suivi équipe en temps réel', 'Rapports de performance', 'Support dédié 24h/24'], highlight: false },
];
const testimonials = [
    ...
    <main className="min-h-screen bg-slate-950 text-white">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => handleSubscribe('pro')} disabled={loading !== null} className="text-sm bg-emerald-600 hover:bg-emerald-500 disabled:bg-emerald-800 text-white font-semibold px-4 py-2 rounded-lg transition flex items-center gap-1.5">
      ...
      <section className="pt-32 pb-20 px-4 text-center">
        <div className="max-w-4xl mx-auto">
          <div className="inline-flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm font-medium px-4 py-1.5 rounded-full mb-8">
            <Zap className="w-3.5 h-3.5" /> Propulsé par IA — Conforme RGPD — Made in EU
          </div>
          <h1 className="text-5xl md:text-7xl font-black tracking-tight leading-none mb-6">
            Ton coach fitness <br /> <span className="text-emerald-400">100 % personnalisé</span>
          </h1>
          <p className="text-xl text-slate-400 max-w-2xl mx-auto mb-10 leading-relaxed">
            RegenX génère tes programmes d’entraînement et tes plans nutritionnels sur mesure.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-12">
            <button onClick={() => handleSubscribe('pro')} disabled={loading !== null} className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 disabled:bg-emerald-800 text-white font-bold px-8 py-4 rounded-xl text-lg transition">
          ...
          <div className="flex flex-wrap items-center justify-center gap-6 text-sm text-slate-500">
            {['Remboursé si rétractation sous 14 jours', 'Résiliable à tout moment', 'Conforme RGPD'].map((t) => (
              <span key={t} className="flex items-center gap-1.5"><CheckCircle className="w-4 h-4 text-emerald-500" /> {t}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="py-8 border-y border-white/5">
        <div className="max-w-4xl mx-auto px-4 flex flex-wrap justify-center gap-12 text-center">
          {[['2 000+', 'Athlètes actifs'], ['98 %', 'Taux de satisfaction'], ['4,9/5', 'Note moyenne'], ['EU', 'Hébergement']].map(([n, l]) => (
            <div key={l}><div className="text-3xl font-black text-emerald-400">{n}</div><div className="text-sm text-slate-500 mt-1">{l}</div></div>
          ))}
        </div>
      </section>
      {/* Features */}
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {testimonials.map((t) => (
              <div key={t.name} className="bg-white/3 border border-white/8 rounded-2xl p-6">
                <div className="flex gap-1 mb-4">{Array.from({length: t.stars}).map((_, i) => (<Star key={i} className="w-4 h-4 fill-emerald-400 text-emerald-400" />))}</div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div><div className="font-semibold text-sm">{t.name}</div><div className="text-slate-500 text-xs">{t.role}</div></div>
                </div>
              </div>
            ))}
          </div>
      ...
      <section id="pricing" className="py-24 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {plans.map((p) => (
              <div key={p.name} className={'relative rounded-2xl p-7 border transition-all duration-200 ' + (p.highlight ? 'bg-emerald-600/10 border-emerald-500/40 ring-1 ring-emerald-500/30 shadow-2xl' : 'bg-white/3 border-white/8')}>
                {p.badge && (<div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-emerald-500 text-white text-xs font-bold px-3 py-1 rounded-full">{p.badge}</div>)}
                <div className="mb-4"><div className="font-bold text-lg">{p.name}</div><div className="text-slate-400 text-sm">{p.desc}</div></div>
                ...
                <ul className="space-y-2 mb-7">{p.features.map((feat) => (<li key={feat} className="flex items-start gap-2 text-sm text-slate-300"><CheckCircle className="w-4 h-4 text-emerald-400 mt-0.5 shrink-0" />{feat}</li>))}</ul>
                <button onClick={() => handleSubscribe(p.key)} disabled={loading !== null} className={'flex items-center justify-center gap-2 w-full font-semibold py-3 rounded-xl transition ' + (p.highlight ? 'bg-emerald-600 hover:bg-emerald-500 disabled:bg-emerald-800 text-white' : 'bg-white/8 hover:bg-white/12 disabled:opacity-50 text-white border border-white/10')}>
          ...
          <p className="text-center text-xs text-slate-500 mt-8">Tous les prix incluent la TVA — Résiliable à tout moment — Remboursé si rétractation sous 14 jours — Conforme RGPD</p>
        </div>
      </section>
      {/* CTA */}
      <section className="py-24 px-4">
        <div className="max-w-3xl mx-auto text-center">
          <div className="bg-gradient-to-br from-emerald-900/40 to-slate-900/40 border border-emerald-500/20 rounded-3xl p-12">
            ...
            <p className="text-slate-400 text-lg mb-8">Rejoins les 2 000+ athlètes qui ont transformé leur corps avec RegenX.<br /> Remboursé si rétractation dans les 14 jours suivant l’abonnement.</p>
            <button onClick={() => handleSubscribe('pro')} disabled={loading !== null} className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 disabled:bg-emerald-800 text-white font-bold px-10 py-4 rounded-xl text-lg transition">
              {loading === 'pro' ? <Loader2 className="w-5 h-5 animate-spin" /> : <ArrowRight className="w-5 h-5" />}
