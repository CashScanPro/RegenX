  ...
  return (
    <div className="min-h-screen flex" style={{ background: '#0a0a0a' }}>
      ...
      <div className="flex flex-col flex-1" style={{ marginLeft: '0' }}>
        ...
        <div>
        <h1 style={{ color: '#fff', fontFamily: SERIF, fontWeight: 600, fontSize: '1.25rem', letterSpacing: '0.01em' }}>Coach IA RegenX</h1>h1>
        ...
        <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: GOLD, display: 'inline-block' }} className="animate-pulse"></span>span> En ligne
        </p>p>
        </div>div>
        </header>header>
      ...
      <div className="whitespace-pre-wrap" style={Object.assign({ maxWidth: '80%', padding: '0.95rem 1.2rem', borderRadius: '4px', fontSize: '0.88rem', lineHeight: 1.7, fontWeight: 300 }, m.role === 'user' ? { background: 'linear-gradient(135deg, ' + GOLD + ', ' + GOLD_LIGHT + ')', color: '#0a0a0a', fontWeight: 500 } : { background: '#141414', border: '1px solid rgba(255,255,255,0.06)', color: '#e5e5e5' })}>
        {m.content || (loading && i === messages.length - 1 ? <span className="flex gap-1 items-center"><span className="w-1.5 h-1.5 rounded-full animate-bounce" style={{ background: GOLD, animationDelay: '0ms' }}></span>span><span className="w-1.5 h-1.5 rounded-full animate-bounce" style={{ background: GOLD, animationDelay: '150ms' }}></span>span><span className="w-1.5 h-1.5 rounded-full animate-bounce" style={{ background: GOLD, animationDelay: '300ms' }}></span>span></span>span> : '')}
      </div>div>
        {m.role === 'user' && (
        <div style={{ width: '34px', height: '34px', borderRadius: '4px', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: '4px', fontWeight: 600, color: GOLD, background: 'rgba(200,146,42,0.1)', border: '1px solid rgba(200,146,42,0.2)' }}>T</div>div>
      )}
      </div>div>
      ))}
          {error && (
      ...
      <span>{error}</span>span><Link href="/pricing" style={{ color: GOLD, fontWeight: 600, fontSize: '0.78rem' }}>Abonnement →</Link>Link>
      </div>div>
        )}
        <div ref={endRef} />
        </div>div>
